...
For performance reasons, the SSSD works in a completely non-blocking way using the tevent event loop library from the Samba project. To integrate the DBus API with the event loop and provide a level of abstraction, the SSSD uses a wrapper around the D-Bus library called the S-Bus. The S-Bus code can be found in the [src/sbus](https://github.com/SSSD/sssd/blob/master/src/sbus) subdirectory. In particular, the wrappers and tevent integration can be found in [sssd_dbus_common.c](https://github.com/SSSD/sssd/blob/master/src/sbus/sssd_dbus_common.c) and [sssd_dbus_connection.c](https://github.com/SSSD/sssd/blob/master/src/sbus/sssd_dbus_connection.c) files. When the message is received, the tevent loop invokes the `sbus_message_handler` function which is located in [sssd_dbus_connection.c](http://https://github.com/SSSD/sssd/blob/master/src/sbus/sssd_dbus_connection.c). The handler selects the SSSD internal interface that receives the message and invokes the appropriate handler.
...
One example of the canonical XML code might be found in the [unit tests](https://github.com/SSSD/sssd/blob/master/src/tests/sbus_codegen_tests.xml), along with the [corresponding autogenerated code](https://github.com/SSSD/sssd/blob/master/src/tests/sbus_codegen_tests_generated.c). The XML files for the internal interfaces, such as the [Data Provider](https://github.com/SSSD/sssd/blob/master/src/providers/data_provider_iface.xml) can also be inspected. Since all the internal interfaces use the raw approach, the autogenerated code is [quite terse](https://github.com/SSSD/sssd/blob/master/src/providers/data_provider_iface_generated.c) and the [interface handlers](https://github.com/SSSD/sssd/blob/master/src/providers/data_provider_be.c) do the packing and unpacking on their own.
...
The SSSD might discover additional KDC or Kadmin servers that are not defined in krb5.conf. However, it would still be prudent if tools like kinit or kpasswd could talk to the same servers the SSSD talks to. To this end, the SSSD implements a plugin for libkrb5, located in the [sssd_krb5_locator_plugin.c](https://github.com/SSSD/sssd/blob/master/src/krb5_plugin/sssd_krb5_locator_plugin.c) file. When a new KDC is discovered, the sssd_be process writes the IP address of this KDC into a file under the /var/lib/sss/pubconf directory. With the help of the locator plugin, libkrb5 is able to read these files in the pubconf directory and use the KDC servers discovered by the SSSD.
